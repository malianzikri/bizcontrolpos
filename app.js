...
const ROLE_PERMISSIONS={
  ...
  cashier:['dashboard_basic','sales_view','sales_create','payments_manage','products_view','print','sync'],
...
function renderTeam(){
  ...
  return `<div class="role-guide"><div><b>Owner</b><span>Akses penuh & pengaturan tim</span></div><div><b>Admin</b><span>Operasional + finansial, tanpa kelola anggota</span></div><div><b>Kasir</b><span>POS, shift, pembayaran & member</span></div><div><b>Finance</b><span>HPP, pembelian, biaya & laporan</span></div><div><b>Gudang</b><span>Stok, opname, recipe & penerimaan</span></div><div><b>Staff</b><span>Produk read-only & dashboard dasar</span></div></div>
...
function openPostSaleSuccessModal(saleId,{change=0,tendered=null,method='Cash'}={}){
  ...
  openModal(`<div class="modal-title"><h2>Transaksi Berhasil</h2><button class="modal-close">×</button></div><div class="document-modal-wrap">
    ...
    <div class="modal-actions"><button type="button" class="ghost" id="postSaleSkipBtn">Tidak Sekarang</button><button type="button" class="ghost" id="postSaleDocsBtn">Dokumen Lain</button>${can('print')?'<button type="button" class="ghost" id="postSaleInvoiceBtn">Invoice A4</button><button type="button" class="primary" id="postSaleThermalBtn">Struk Thermal</button>':''}</div>
  </div>`);
  ...
  $('#postSaleInvoiceBtn')?.addEventListener('click',()=>printSaleDocument(saleId,'invoice'));
  $('#postSaleThermalBtn')?.addEventListener('click',()=>{closeModal();openThermalPrintModal(saleId,{change,tendered:tenderedAmount,method})});
}
...
function openPrintDocumentModal(saleId){
  ...
  openModal(`<div class="modal-title"><h2>Cetak Dokumen Kasir</h2><button class="modal-close">×</button></div><div class="document-modal-wrap">
    ...
    <div class="document-choice-grid">
      ${currentRole()!=='warehouse'?`<button type="button" class="document-choice doc-print-btn" data-doc="invoice"><span class="doc-icon">▤</span><b>Invoice A4</b><small>Format invoice besar untuk dokumen formal, PDF, atau printer biasa.</small><em>Cetak Invoice A4 →</em></button>`:''}
      ${currentRole()!=='warehouse'?`<button type="button" class="document-choice" id="thermalPrintChoice"><span class="doc-icon">▥</span><b>Struk Thermal</b><small>Format ringkas untuk printer kasir 58mm atau 80mm.</small><em>Pilih Ukuran Thermal →</em></button>`:''}
  ...
  $('#thermalPrintChoice')?.addEventListener('click',()=>{closeModal();openThermalPrintModal(saleId)});
  $('#editSaleFromPrint')?.addEventListener('click',()=>{closeModal();openSaleModal(saleId)});
}

function openThermalPrintModal(saleId,printMeta=null){
  const sale=state.sales.find(x=>x.id===saleId&&x.business_id===state.currentBusinessId);if(!sale){toast('Transaksi tidak ditemukan');return;}
  openModal(`<div class="modal-title"><h2>Pilih Ukuran Struk Thermal</h2><button class="modal-close">×</button></div><div class="document-modal-wrap">
    <div class="document-sale-summary"><div><span>Invoice</span><strong>${escapeHtml(sale.invoice_no||'-')}</strong></div><div><span>Total</span><strong>${rupiah(sale.total)}</strong></div><div><span>Customer</span><strong>${escapeHtml(sale.customer||'Umum')}</strong></div></div>
    <div class="document-choice-grid">
      <button type="button" class="document-choice thermal-size-btn" data-size="58"><span class="doc-icon">58</span><b>Thermal 58mm</b><small>Cocok untuk printer kasir mini. Layout lebih padat dan hemat kertas.</small><em>Cetak 58mm →</em></button>
      <button type="button" class="document-choice thermal-size-btn" data-size="80"><span class="doc-icon">80</span><b>Thermal 80mm</b><small>Lebih lega dan mudah dibaca. Cocok untuk sebagian besar printer POS.</small><em>Cetak 80mm →</em></button>
    </div>
    <div class="document-tip">Di dialog printer, pilih printer thermal yang digunakan dan pastikan ukuran kertas sesuai <b>58mm</b> atau <b>80mm</b>. Matikan header/footer browser bila muncul.</div>
    <div class="modal-actions"><button type="button" class="ghost" id="thermalBackBtn">Kembali</button><button type="button" class="primary modal-close">Selesai</button></div>
  </div>`);
  $$('.thermal-size-btn').forEach(btn=>btn.onclick=()=>printSaleDocument(saleId,btn.dataset.size==='58'?'thermal58':'thermal80',null,printMeta));
  $('#thermalBackBtn')?.addEventListener('click',()=>{closeModal();openPrintDocumentModal(saleId)});
}
...
function printSaleDocument(saleId,type,paymentId=null,printMeta=null){
  const sale=state.sales.find(x=>x.id===saleId&&x.business_id===state.currentBusinessId); if(!sale){toast('Transaksi tidak ditemukan');return;}
  ...
  const html=buildSaleDocumentHtml(sale,type,payment,printMeta);
...
function buildSaleDocumentHtml(sale,type,payment=null,printMeta=null){
  ...
  if(type==='thermal58'||type==='thermal80'){
    const paper=type==='thermal58'?58:80; const content=paper===58?52:72; const font=paper===58?10:11;
    const customer=(sale.customer||'').trim(); const phone=(sale.customer_phone||'').trim();
    const tendered=printMeta&&printMeta.tendered!==null&&printMeta.tendered!==undefined?Number(printMeta.tendered||0):null;
    const change=printMeta?Math.max(Number(printMeta.change||0),0):null; const method=printMeta?.method||paymentMethods;
    const itemRows=lines.map(x=>{const lineTotal=Number(x.line_total??(Number(x.qty||0)*Number(x.unit_price||0)));return `<div class="item"><div class="item-name">${docSafe(x.product_name)}</div><div class="item-calc"><span>${num(x.qty)} ${docSafe(x.unit||'pcs')} × ${rupiah(x.unit_price)}</span><b>${rupiah(lineTotal)}</b></div></div>`}).join('');
    const cashRows=method==='Cash'&&tendered!==null?`<div class="money-row"><span>Uang diterima</span><b>${rupiah(tendered)}</b></div><div class="money-row"><span>Kembalian</span><b>${rupiah(change||0)}</b></div>`:'';
    const thermalCss=`<style>@page{size:${paper}mm auto;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif;font-size:${font}px;line-height:1.35}body{width:${paper}mm}.receipt{width:${content}mm;margin:0 auto;padding:3mm 0 5mm}.center{text-align:center}.biz-name{font-size:${paper===58?15:17}px;font-weight:800;line-height:1.15;margin-bottom:2px}.muted{font-size:${paper===58?8.5:9.5}px}.dash{border-top:1px dashed #000;margin:7px 0}.meta{display:grid;grid-template-columns:auto 1fr;gap:2px 6px}.meta span:nth-child(2n){text-align:right;overflow-wrap:anywhere}.item{margin:6px 0}.item-name{font-weight:700;overflow-wrap:anywhere}.item-calc{display:flex;justify-content:space-between;gap:6px;margin-top:1px}.item-calc span{white-space:nowrap}.item-calc b{text-align:right}.money-row{display:flex;justify-content:space-between;gap:8px;margin:2px 0}.money-row.total{font-size:${paper===58?12:14}px;font-weight:800;margin:5px 0}.footer{text-align:center;margin-top:8px;font-size:${paper===58?8.5:9.5}px;overflow-wrap:anywhere}.status-line{text-align:center;font-weight:800;border:1px solid #000;padding:3px;margin:6px 0}.nowrap{white-space:nowrap}@media print{html,body{width:${paper}mm}.receipt{width:${content}mm}.no-print{display:none}}</style>`;
    return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Struk ${docSafe(sale.invoice_no)} · ${paper}mm</title>${thermalCss}</head><body><main class="receipt"><div class="center"><div class="biz-name">${docSafe(b.name||'Nama Bisnis')}</div>${b.address?`<div class="muted">${docSafe(b.address)}</div>`:''}${(b.phone||b.email)?`<div class="muted">${[b.phone,b.email].filter(Boolean).map(docSafe).join(' · ')}</div>`:''}</div><div class="dash"></div><div class="meta"><span>Invoice</span><span>${docSafe(sale.invoice_no||'-')}</span><span>Tanggal</span><span>${date}</span><span>Metode</span><span>${docSafe(method||'-')}</span>${customer?`<span>Customer</span><span>${docSafe(customer)}</span>`:''}${phone?`<span>HP</span><span>${docSafe(phone)}</span>`:''}</div><div class="dash"></div>${itemRows}<div class="dash"></div><div class="money-row"><span>Subtotal</span><b>${rupiah(subtotal)}</b></div>${discount?`<div class="money-row"><span>Diskon</span><b>- ${rupiah(discount)}</b></div>`:''}<div class="money-row total"><span>TOTAL</span><b>${rupiah(total)}</b></div><div class="money-row"><span>Dibayar</span><b>${rupiah(paid)}</b></div>${cashRows}${due?`<div class="money-row"><span>Sisa tagihan</span><b>${rupiah(due)}</b></div>`:''}<div class="status-line">${status}</div>${sale.notes?`<div class="muted"><b>Catatan:</b> ${docSafe(sale.notes)}</div>`:''}<div class="dash"></div><div class="footer">${docSafe(b.document_footer||'Terima kasih atas kunjungan Anda.')}<br><span>BizControl Online · ${docSafe(sale.invoice_no||'')}</span></div></main>${script}</body></html>`;
  }
...
(async()=>{
  ...
  // Direct Demo entry dari landing page.
