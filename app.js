...
const defaultState = () => ({
  ...
  payments:[
    {id:'pay1',business_id:'demo-business',sale_id:'s1',payment_no:1,payment_date:today(),amount:150000,method:'QRIS',notes:'Pembayaran transaksi',created_at:new Date().toISOString()},
...
function metrics(){
  ...
  const hpp=mSales.reduce((a,s)=>a+(Number(s.unit_cost||0)*Number(s.qty||0)),0);
...
function renderSettingsPage(){
  ...
  return `<div class="panel">
    <div class="setting-block"><div class="status-row"><div><h3>Akun & Role</h3><div class="muted">${state.mode==='cloud'?escapeHtml(state.session?.user?.email||'User cloud'):'Mode Demo — tanpa login'}</div></div>${roleBadge()}</div>${state.mode==='cloud'?'<button class="ghost" data-action="logout">Keluar dari Akun</button>':'<button class="soft-btn" data-action="open-auth">Masuk / Buat Akun</button>'}</div>
    ...
    <div class="setting-block"><h3>Versi</h3><div class="status-row"><span>BizControl Online</span><span class="code-chip">V1.8.5 Production Final</span></div></div>
  </div>`;
}
...
function openSaleModal(saleId=null){
  ...
  const selectedProductId=existing?.product_id||products[0]?.id;
  ...
  <form id="saleForm" class="form-grid">
    ...
    <label>No. Invoice<input name="invoice_no" value="${escapeAttr(existing?.invoice_no||'Otomatis saat disimpan')}" readonly><small class="micro">Nomor unik otomatis.</small></label>
    ...
    <label>Produk<select name="product_id" id="saleProduct">${products.map(p=>`<option value="${p.id}" ${p.id===selectedProductId?'selected':''}>${escapeHtml(p.sku)} — ${escapeHtml(p.name)}</option>`).join('')}</select></label>
    <label>Qty<input name="qty" type="number" min="0.01" step="0.01" value="${existing?.qty??1}" required></label>
    <label>Diskon<input name="discount" type="number" min="0" value="${existing?.discount??0}"></label>
    <label>Metode Bayar${editing?`<input value="${escapeAttr(existing?.payment_method||'Lihat Riwayat Pembayaran')}" readonly><small class="micro">Metode transaksi awal. Pembayaran berikutnya dicatat terpisah.</small>`:`<select name="payment_method" id="salePaymentMethod">${payMethods.map(v=>`<option ${v===(existing?.payment_method||'Cash')?'selected':''}>${v}</option>`).join('')}</select>`}</label>
    <label>${editing?'Sudah Dibayar':'Pembayaran Awal'}<input ${editing?'':'name="paid_amount"'} id="salePaidAmount" type="number" min="0" value="${editing?Number(existing?.paid_amount||0):''}" placeholder="Tempo kosong = Rp0" ${editing?'readonly':''}><small class="micro">${editing?'Dihitung otomatis dari Riwayat Pembayaran.':'Jika ada DP/pembayaran awal, sistem otomatis membuat Riwayat Pembayaran #1.'}</small></label>
    ...
    <div class="span-2 payment-status-box"><div><span>Status Pembayaran</span><strong id="saleStatusText">—</strong><small>Status otomatis dari Total vs seluruh Riwayat Pembayaran.</small></div><button type="button" class="soft-btn" id="markPaidBtn">${editing?'Bayar Sisa / Tandai Lunas':'Tandai Lunas'}</button></div>
  ...
  const calc=()=>{
    ...
    const q=Number(form.qty.value||0),disc=Number(form.discount.value||0);
    const effectivePrice=editing&&p?.id===existing?.product_id?Number(existing.unit_price||p?.price||0):Number(p?.price||0);
    const total=Math.max(q*effectivePrice-disc,0);
    const paidInput=$('#salePaidAmount').value;
    const method=editing?(existing?.payment_method||'Tempo'):$('#salePaymentMethod').value;
    ...
    $('#salePreview').innerHTML=`Harga ${rupiah(effectivePrice)} × ${q} − diskon ${rupiah(disc)} = <strong>${rupiah(total)}</strong> · Stok ${p?.category==='Jasa'?'Jasa':num(p?.stock)}`;
    $('#saleStatusText').textContent=due>0?`Belum Lunas · Sisa ${rupiah(due)}`:'Lunas';
    $('#saleStatusText').className=due>0?'status-warn':'status-good';
    return {p,q,disc,total,effectivePaid};
  };
  form.oninput=calc; form.onchange=calc;
  ...
  form.onsubmit=async e=>{
    e.preventDefault();
    const f=new FormData(form); const p=products.find(x=>x.id===f.get('product_id'));
    const qty=Number(f.get('qty')),discount=Number(f.get('discount')||0); const effectivePrice=editing&&p.id===existing.product_id?Number(existing.unit_price||p.price):Number(p.price); const effectiveCost=editing&&p.id===existing.product_id?Number(existing.unit_cost||p.cost):Number(p.cost); const total=Math.max(qty*effectivePrice-discount,0);
    const rawPaid=editing?String(existing?.paid_amount||0):f.get('paid_amount'); const saleMethod=editing?(existing?.payment_method||'Tempo'):f.get('payment_method'); let paid=editing?Number(existing?.paid_amount||0):(rawPaid===''?(saleMethod==='Tempo'?0:total):Number(rawPaid||0));
    if(editing&&paid>total){toast('Total transaksi baru lebih kecil dari pembayaran yang sudah tercatat. Edit/hapus Riwayat Pembayaran terlebih dahulu.');return;}
    paid=Math.max(0,Math.min(paid,total));
    let available=Number(p.stock||0);
    if(editing&&existing.product_id===p.id){ const oldP=products.find(x=>x.id===existing.product_id); if(oldP?.category!=='Jasa') available+=Number(existing.qty||0); }
    if(p.category!=='Jasa'&&!activeBusiness()?.allow_negative_stock&&qty>available){toast('Stok tidak cukup untuk perubahan ini');return;}
    const sale={business_id:state.currentBusinessId,client_request_id:submissionRequestId,date:f.get('date'),invoice_no:editing?existing.invoice_no:null,delivery_no:editing?existing.delivery_no:null,customer:f.get('customer'),customer_phone:f.get('customer_phone'),customer_address:f.get('customer_address'),notes:f.get('notes'),product_id:p.id,product_name:p.name,qty,unit_price:effectivePrice,unit_cost:effectiveCost,discount,total,gross_profit:total-(qty*effectiveCost),payment_method:saleMethod,paid_amount:paid};
...
function buildSaleDocumentHtml(sale,type,payment=null){
  const b=activeBusiness()||{}; const p=state.products.find(x=>x.id===sale.product_id)||{};
  const qty=Number(sale.qty||0), price=Number(sale.unit_price||0), discount=Number(sale.discount||0), total=Number(sale.total||0), paid=Number(sale.paid_amount||0), due=Math.max(total-paid,0);
  const subtotal=qty*price; const unit=p.unit||'pcs'; const city=b.city||''; const date=fmtDate(sale.date); const status=due>0?'BELUM LUNAS':'LUNAS'; const paymentRows=paymentsForSale(sale.id); const paymentMethods=[...new Set(paymentRows.map(x=>x.method).filter(Boolean))].join(', ')||sale.payment_method||'-';
...
async function addSale(sale){
  if(!requirePermission('sales_create'))throw new Error('Tidak diizinkan');const requestId=sale.client_request_id||newClientRequestId();
  if(state.mode==='cloud'){
    const result=await cloudRequest('/rest/v1/rpc/create_sale_with_payment',{method:'POST',body:{p_bid:state.currentBusinessId,p_sale_date:sale.date,p_customer_name:sale.customer||null,p_customer_phone:sale.customer_phone||null,p_customer_address:sale.customer_address||null,p_sale_notes:sale.notes||null,p_product_id:sale.product_id,p_qty:Number(sale.qty||0),p_discount:Number(sale.discount||0),p_payment_method:sale.payment_method||'Cash',p_initial_paid:Number(sale.paid_amount||0),p_request_id:requestId}});
    if(result?.ok===false)throw new Error(result.error||'Gagal membuat transaksi');
    await cloudLoadBusinessData();
  } else {
    sale.invoice_no=sale.invoice_no||await nextDocumentNumber('INV',sale.date);sale.delivery_no=sale.delivery_no||await nextDocumentNumber('SJ',sale.date);
    const p=state.products.find(x=>x.id===sale.product_id);if(p&&p.category!=='Jasa')p.stock=Number(p.stock)-Number(sale.qty);const initialPaid=Number(sale.paid_amount||0);const row={...sale,id:uid(),client_request_id:requestId,paid_amount:0,created_at:new Date().toISOString()};state.sales.push(row);localAudit('CREATE','sales',row.id,`${row.invoice_no} · ${row.customer||row.product_name}`,null,row);if(initialPaid>0)await addPayment({business_id:state.currentBusinessId,client_request_id:requestId,sale_id:row.id,payment_date:row.date,amount:initialPaid,method:row.payment_method,notes:'Pembayaran awal'});persist();
  }
}
...
async function updateSale(id,sale){if(!requirePermission('sales_edit'))throw new Error('Tidak diizinkan');
  if(state.mode==='cloud'){
    const result=await cloudRequest('/rest/v1/rpc/update_sale_secure',{method:'POST',body:{p_bid:state.currentBusinessId,p_sale_id:id,p_sale_date:sale.date,p_customer_name:sale.customer||null,p_customer_phone:sale.customer_phone||null,p_customer_address:sale.customer_address||null,p_sale_notes:sale.notes||null,p_product_id:sale.product_id,p_qty:Number(sale.qty||0),p_discount:Number(sale.discount||0),p_payment_method:sale.payment_method||'Tempo'}});
    if(result?.ok===false)throw new Error(result.error||'Gagal mengubah transaksi'); await cloudLoadBusinessData();
  } else {
    const old=state.sales.find(x=>x.id===id&&x.business_id===state.currentBusinessId); if(!old) throw new Error('Transaksi tidak ditemukan');
    const before=cloneAuditData(old); const oldP=state.products.find(x=>x.id===old.product_id), newP=state.products.find(x=>x.id===sale.product_id);
    if(oldP&&oldP.category!=='Jasa') oldP.stock=Number(oldP.stock||0)+Number(old.qty||0);
    if(newP&&newP.category!=='Jasa') newP.stock=Number(newP.stock||0)-Number(sale.qty||0);
    Object.assign(old,sale); localAudit('UPDATE','sales',old.id,`${old.invoice_no} · ${old.customer||old.product_name}`,before,old); persist();
  }
...
async function secureDeleteRecord(entity,id,pin){
  ...
  if(entity==='sale'){
    ...
    const p=state.products.find(x=>x.id===row.product_id); if(p&&p.category!=='Jasa') p.stock=Number(p.stock||0)+Number(row.qty||0);
    (state.payments||[]).filter(x=>x.sale_id===row.id&&x.business_id===state.currentBusinessId).forEach(pay=>localAudit('DELETE','payments',pay.id,`${row.invoice_no} · Pembayaran #${pay.payment_no}`,pay,null)); state.payments=(state.payments||[]).filter(x=>x.sale_id!==row.id); localAudit('DELETE','sales',row.id,`${row.invoice_no} · ${row.customer||row.product_name}`,row,null); state.sales=state.sales.filter(x=>x.id!==id);
  }else if(entity==='product'){
    if(state.sales.some(x=>x.product_id===id&&x.business_id===state.currentBusinessId)) throw new Error('Produk sudah punya riwayat penjualan. Demi histori laporan, produk ini tidak bisa dihapus. Edit datanya saja.');
  ...
  try{
    ...
    if(!state.businesses.length){
      ...
      if(accountType==='owner'){
        ...
        stopPolling();state.currentBusinessId=null;state.currentRole='staff';state.products=[];state.sales=[];state.payments=[];state.expenses=[];state.auditLogs=[];state.teamMembers=[];
...
async function cloudLoadBusinessData(){
  ...
  const [products,sales,payments,expenses,auditLogs]=await Promise.all(queries);state.products=products||[];state.sales=sales||[];state.payments=payments||[];state.expenses=expenses||[];state.auditLogs=auditLogs||[];await cloudLoadTeam();state.lastSync=new Date().toISOString();setSyncState('idle');
}
...
function exportAllCsv(){if(!can('export')&&state.mode!=='local'){toast('Role ini tidak diizinkan export','error');return}exportCsv('produk',businessData(state.products));setTimeout(()=>exportCsv('penjualan',businessData(state.sales)),250);setTimeout(()=>exportCsv('pembayaran',businessData(state.payments||[])),500);setTimeout(()=>exportCsv('biaya',businessData(state.expenses)),750)}
function importJson(){const i=document.createElement('input');i.type='file';i.accept='application/json';i.onchange=()=>{const f=i.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const data=JSON.parse(r.result);state={...defaultState(),...data,mode:'local',session:null};persist();render();toast('Backup diimport','success')}catch{toast('File backup tidak valid','error')}};r.readAsText(f)};i.click()}
